...
#define SURF_BPP 3
#define SURF_SIZE(X, Y)                                                        \
    (((uint16_t)ceil((float)(SURF_BPP * ((X) * (Y))) / 8.0) % SURF_BPP) * 3 +  \
     ((SURF_BPP * ((X) * (Y))) / 8))
#define SURF_POSITION(SURF, X, Y) ((Y) * (SURF)->width + (X))
#define PIXEL_MASK ((1 << SURF_BPP) - 1)

// rgb to gpio config
#define COLOR_TO_GPIO(red, green, blue)                                        \
...
// gpio config to rgb
#define RED_FROM_GPIO(color)                                                   \
    (((color << 1) & 0b100) | ((color >> 9) & 0b010) | ((color >> 11) & 0b001))
#define GREEN_FROM_GPIO(color)                                                 \
    (((color >> 10) & 0b100) | ((color >> 12) & 0b010) |                       \
     ((color >> 14) & 0b001))
#define BLUE_FROM_GPIO(color)                                                  \
    (((color >> 5) & 0b100) | ((color >> 7) & 0b010) | ((color >> 9) & 0b001))
...
static Surface surf_create_from_memory(uint8_t width, uint8_t height,
                                       void *data) {
...
static void surf_destroy(Surface *surf) { free(surf->data); }
...
static void surf_draw_line(Surface *surf, int x0, int y0, int x1, int y1,
                           uint8_t color) {
...
static void surf_draw_circle(Surface *surf, int xm, int ym, int r,
                             uint8_t color) {
...
static void surf_draw_ellipse(Surface *surf, int xm, int ym, int a, int b,
                              uint8_t color) {
    int x = -a, y = 0;
...
static void surf_fill(Surface *surf, uint8_t color) {
    if (color == 0b000) {
        memset(surf->data, 0x00, SURF_SIZE(surf->width, surf->height));
        return;
    } else if (color == 0b111) {
        memset(surf->data, 0xFF, SURF_SIZE(surf->width, surf->height));
        return;
    }
...
static void surf_draw_surf(Surface *destination_surf, Surface *source_surf,
                           uint8_t x, uint8_t y) {
    for (int i = 0; i < source_surf->height; i++) {
        for (int j = 0; j < source_surf->width; j++) {
            surf_set_pixel(destination_surf, j + x, i + y,
                           surf_get_pixel(source_surf, j, i));
        }
    }
}

static void surf_draw_subsurf(Surface *destination_surf, Surface *source_surf,
                              uint8_t dx, uint8_t dy, uint8_t sx, uint8_t sy,
                              uint8_t w, uint8_t h) {
    for (int i = 0; i < h; i++) {
        for (int j = 0; j < w; j++) {
...
static void surf_draw_masked_surf(Surface *destination_surf,
                                  Surface *source_surf, uint8_t x, uint8_t y,
                                  uint8_t mask_color) {
    for (int i = 0; i < source_surf->height; i++) {
        for (int j = 0; j < source_surf->width; j++) {
...
static void surf_draw_masked_subsurf(Surface *destination_surf,
                                     Surface *source_surf, uint8_t dx,
                                     uint8_t dy, uint8_t sx, uint8_t sy,
                                     uint8_t w, uint8_t h, uint8_t mask_color) {
    for (int i = 0; i < h; i++) {
        for (int j = 0; j < w; j++) {
            uint8_t color = surf_get_pixel(source_surf, j + sx, i + sy);
            if (color != mask_color) {
...
static void surf_draw_rectangle(Surface *surf, uint8_t x, uint8_t y,
...
static void surf_draw_filled_rectangle(Surface *surf, uint8_t x, uint8_t y,
